...
**_NOTE:_** The difference between `peage-pay-server` and `peage-pay-server-local` is that `peage-pay-server-local` is a simplified version that does dot require **Redis**
...
For the payment system to work you have to configure the webhook url for the **_Chargily Pay_** API in the dashboard
