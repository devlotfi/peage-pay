...
const ModeratorListPage = (): JSX.Element => {
  ...
  const { data, loading, error } = useQuery(MODERATOR_LIST, {
    variables: {
      moderatorListInput: {
        take: 10,
        skip: 10 * (page - 1),
    ...
    <ListPageLayout>
      ...
      <AdminDashboardLayout.Loading loading={loading}>
        <AdminDashboardLayout.Error error={error}>
          <ListPageLayout.Empty list={data?.moderatorList.list}>
            <Table.Container className="h-full">
              <Table>
                ...
                <Table.Body>
                  {data?.moderatorList.list.map((moderator) => (
                    <ModeratorItem
                      key={moderator.baseUser.id}
                      moderator={moderator as ModeratorType}
                    ></ModeratorItem>
                  ))}
                </Table.Body>
              </Table>
        ...
        <ListPageLayout.Footer>
          {data ? (
            <Pagination
              value={page}
              maxPages={Math.ceil(data.moderatorList.count / 10)}
