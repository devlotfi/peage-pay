...
const GateAdminListPage = (): JSX.Element => {
  const { t } = useTranslation();
  const { renderFieldOptions } = useRenderFieldOptions();
  const [searchData, setSearchData] = useState(initialValues);
  const [page, setPage] = useState<number>(1);

  const { data, loading, error } = useQuery(GATE_ADMIN_LIST, {
    variables: {
      gateAdminListInput: {
        take: 10,
        skip: 10 * (page - 1),
    ...
    <ListPageLayout>
      ...
      <ListPageLayout.Title>
        <Heading className="text-[20pt]">
          ...
          <Heading.Text>{t('GATE_ADMIN_LIST')}</Heading.Text>
        </Heading>
      </ListPageLayout.Title>

      <AdminDashboardLayout.Loading loading={loading}>
        <AdminDashboardLayout.Error error={error}>
          <ListPageLayout.Empty list={data?.gateAdminList.list}>
            <Table.Container className="h-full">
              <Table>
                ...
                <Table.Body>
                  {data?.gateAdminList.list.map((gateAdmin) => (
                    <GateAdminItem
                      key={gateAdmin.baseUser.id}
                      gateAdmin={gateAdmin as GateAdminType}
                    ></GateAdminItem>
                  ))}
                </Table.Body>
              </Table>
        ...
        <ListPageLayout.Footer>
          {data ? (
            <Pagination
              value={page}
              maxPages={Math.ceil(data.gateAdminList.count / 10)}
...
export default GateAdminListPage;
