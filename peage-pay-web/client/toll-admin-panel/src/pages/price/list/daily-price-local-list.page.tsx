...
const DailyPriceLocalListPage = (): JSX.Element => {
  ...
  const {
    ...
    loading: listLoading,
    error: listError,
  } = useQuery(DAILY_PRICE_LOCAL_LIST, {
    variables: {
      priceListInput: {
        take: 10,
        skip: 10 * (page - 1),
    ...
    <ListPageLayout>
      <ListPageLayout.Title>
        <Heading className="text-[20pt]">
          ...
          <Heading.Text>{t('DAILY_PRICE_LOCAL_LIST')}</Heading.Text>
        </Heading>
      </ListPageLayout.Title>

      <AdminDashboardLayout.Loading loading={listLoading}>
        <AdminDashboardLayout.Error error={listError}>
          <ListPageLayout.Empty list={listData?.dailyPriceLocalList.list}>
            <Table.Container className="h-full">
              <Table>
                ...
                <Table.Body>
                  {listData?.dailyPriceLocalList.list.map((dailyPrice) => (
                    <DailyPriceListItem
      ...
      <ListPageLayout.Footer>
        {listData ? (
          <Pagination
            value={page}
            maxPages={Math.ceil(listData.dailyPriceLocalList.count / 10)}
...
export default DailyPriceLocalListPage;
